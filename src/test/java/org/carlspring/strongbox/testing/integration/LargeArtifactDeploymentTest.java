        ...
        extends RestAssuredBaseTest
{
                   ...
                   ArtifactTransportException
    {
        ...
        artifactDeployer.setSize(200000000L); // ~20 MB
        artifactDeployer.generateAndDeployArtifact(artifact1WithTimestamp1, storageId, repositoryId);

        File artifactBaseDir = new File(BASEDIR_SNAPSHOTS + "/org/carlspring/strongbox/tests/large-artifact-2GB/1.0-SNAPSHOT");

        assertTrue(new File(artifactBaseDir, "large-artifact-2GB-" + snapshotVersion + ".jar").exists());
        assertTrue(new File(artifactBaseDir, "large-artifact-2GB-" + snapshotVersion + ".jar.md5").exists());
        assertTrue(new File(artifactBaseDir, "large-artifact-2GB-" + snapshotVersion + ".jar.sha1").exists());
        assertTrue(new File(artifactBaseDir, "large-artifact-2GB-" + snapshotVersion + ".pom").exists());
        assertTrue(new File(artifactBaseDir, "large-artifact-2GB-" + snapshotVersion + ".pom.md5").exists());
        assertTrue(new File(artifactBaseDir, "large-artifact-2GB-" + snapshotVersion + ".pom.sha1").exists());
                   ...
                   ArtifactTransportException
    {
        ...
        File artifactBaseDir = new File(ConfigurationResourceResolver.getVaultDirectory() + "/" +
                                        "storages/storage0/releases/" +
                                        "org/carlspring/strongbox/tests/large-artifact-2GB/1.0");

        assertTrue(new File(artifactBaseDir, "large-artifact-2GB-1.0.jar").exists());
        assertTrue(new File(artifactBaseDir, "large-artifact-2GB-1.0.jar.md5").exists());
        assertTrue(new File(artifactBaseDir, "large-artifact-2GB-1.0.jar.sha1").exists());
        assertTrue(new File(artifactBaseDir, "large-artifact-2GB-1.0.pom").exists());
        assertTrue(new File(artifactBaseDir, "large-artifact-2GB-1.0.pom.md5").exists());
        assertTrue(new File(artifactBaseDir, "large-artifact-2GB-1.0.pom.sha1").exists());
        assertTrue(new File(artifactBaseDir, "maven-metadata.xml").exists());
        assertTrue(new File(artifactBaseDir, "maven-metadata.xml.md5").exists());
        assertTrue(new File(artifactBaseDir, "maven-metadata.xml.sha1").exists());
